...
class TestAmsatsSatNOGS(unittest.TestCase):
    ...
    def test_item_to_memory_mode_mapping(self):
        ...
        for orig_mode, expected_mode in test_cases:
            ...
            self.assertEqual(mem.mode, expected_mode,
                             f"Failed mapping {orig_mode}")
            ...
            # Check for fixed URL
            self.assertIn("https://db.satnogs.org/satellite/12345",
                          mem.comment)
    ...
    def test_do_fetch_filtering(self, mock_fetch):
        ...
        mock_fetch.side_effect = [
            # Satellites
            [{'sat_id': 'SAT1', 'name': 'Sat 1', 'norad_cat_id': '1'}],
            # Transmitters
            [
...
class TestAmsatsRadioAmateur(unittest.TestCase):
    def test_item_to_memory_restyling(self):
        ...
        self.assertFalse(mem.comment.startswith("NORAD:"))
        # RadioAmateurSatellites still has Mode: in comment as per my
        # implementation
        self.assertIn("Mode:FM", mem.comment)
