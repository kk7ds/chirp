...
from chirp.settings import RadioSetting, RadioSettingGroup, \
    ...
    RadioSettingValueInteger, RadioSettingValueMap, RadioSettingValueString, \
    RadioSettings
...
#
# Exactly one priority scan channel is supported.  It cannot be unset.
# If multiple memory locations are marked for priority scan, the last
# one to be written will be the priority channel.
#
# The radio supports three AM modes, but CHIRP only knows about two,
# so WAM on the radio is mapped to AM on read, and cannot be set with
# CHIRP.
#
...
# port.
#
# Unclear what concurrency guarantees are necessary in class methods.
# Notably, if one tries to update settings while downloading memory,
# there could be inconsistent information returned for search banks.
#
# The radio allows arbitrary tuning steps to be set, in the range .05
# kHz to 999.95 kHz in 50 Hz increments.  Providing all valid steps in
# valid_tuning_steps is unwieldy, so only a variety of common steps
# are used.
#
...
class AR8200Radio(chirp_common.LiveRadio):
    ...
    _MODES = [('WFM', 0), ('FM', 1), ('AM', 2), ('USB', 3), ('LSB', 4),
              ('CW', 5), ('NFM', 6), ('AM', 7), ('NAM', 8), ('Auto', 0xf)]
    ...
    def get_features(self):
        ...
        rf.valid_modes = list(zip(*self._MODES))[0]
        ...
        rf.valid_tones = []
        rf.valid_tuning_steps = sorted(chirp_common.TUNING_STEPS + (.05,))
    ...
    def _command_unlocked(self, cmd, pattern='$', lines=1, cache=False,
    ...
    def _command(self, cmd, pattern='$', lines=1, retries=5, timeout=None):
        with self.LOCK:
            return self._command_unlocked(cmd=cmd, pattern=pattern,
                                          lines=lines, retries=retries,
                                          timeout=timeout)

    def _command_read_WM_unlocked(self, bank, retries):
        retry_wait = .25
    ...
    def _command_read_WM(self, bank, retries=5):
        with self.LOCK:
            return self._command_read_WM_unlocked(bank, retries)

    def _command_update_cache(self, cmd, lines=10, varlist=False, retries=5):
        with self.LOCK:
            return self._command_unlocked(cmd=cmd, lines=lines, cache=True,
    ...
    def _get_pp(self):
        ...
        self._PP = (res[1], int(res[2]))
        LOG.info("Priority channel is {}{:02d}".format(*self._PP))
    ...
    def get_memory(self, number):
        ...
        if region == 'vfo':
            ...
            if not res:
                ...
                mem.mode = 'Auto' if int(res['auto']) else \
                    [mode[0] for mode in self._MODES
            ...
            mem.comment = 'Quick Memo {}'.format(chan)
            ...
            if not res:
                ...
                mem.mode = 'Auto' if int(res['auto']) else \
                    [mode[0] for mode in self._MODES
                     if mode[1] == int(res['mode'])][0]
                mem.tuning_step = float(res['step']) / 1000
                mem.skip = 'P' if (bank, chan) == pp else \
                    'S' if int(res['pass']) else ''
            ...
            mem.comment = '{}{:02d}'.format(bank, chan)
            ...
            mem.comment = "Search Bank {} Pass".format(bank)
            ...
            mem.comment = 'Search Bank {}'.format(bank)
        ...
        mem.immutable += ['extra.selectindex', 'comment', 'tmode', 'rtone']
    ...
    def validate_memory(self, memory):
        ...
        if region == 'scan':
            if memory.skip == 'P' and pp and pp != (bank, chan):
                msg = chirp_common.ValidationWarning(
                    "Priority channel already set")
                msgs.append(msg)

            if (memory.skip == 'P') != (pp == (bank, chan)):
                msg = chirp_common.ValidationWarning(
                    "Memories must be refreshed after changing the priority "
                    "channel")
                msgs.append(msg)
    ...
    def set_memory(self, memory):
        ...
        if region == 'vfo':
            ...
            mode = 0 if memory.mode == 'Auto' else \
                [mode[1] for mode in self._MODES
            ...
            if memory.skip == 'P':
                res = self._command('PP{}{:02d}'.format(bank, chan))
                if not res:
                    raise errors.InvalidDataError(
                        "Error setting {} as priority channel"
                        "".format(memory.number))

                self._invalidate_pp()
                self._invalidate_memory('scan', pp[0], pp[1])
