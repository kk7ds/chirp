...
def _get_memory(radio, mem, _mem, ch_index, isvfo=False):
    ...
    if chirp_common.in_range(mem.freq, [radio._airband]):
        mem.mode = "AM"
...
def _set_memory(radio, mem, _mem, ch_index, isvfo=False):
    if not isvfo:
        ch_index_dict = retevis_ha1g.get_ch_index(radio)
...
def _get_vfo_scan(radio, vfoscan):
    _vfo_scan = radio._memobj.vfoscans.vfoscans[0]
...
def _get_ana_aprs_setting(radio, aprs):
    _aprsinfo = radio._memobj.aprsinfo
    aprs.append(
        RadioSetting(
...
def _get_gnss_setting(radio, gnss):
...
def _get_fm_setting(radio, fm):
    _fminfo = radio._memobj.fmdata.fms
    _settings = radio._memobj.settings
    fm.append(
        RadioSetting(
...
def _get_radio_alias_setting(radio, radioalias):
    _radioaliasinfo = radio._memobj.aliaslist.alias
    alias_len = 8
...
def _get_model_info(radio, model_info):
    rs_value = RadioSettingValueString(0, 20, radio.current_model)
