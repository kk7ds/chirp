...
def _get_common_setting(self, common):
    ...
    opts = ["OFF", "Roger Beep", "MDC Frog Call"]
    common.append(
        RadioSetting(
    ...
    opts = ["Alias-%s" % x for x in range(1, 13, 1)]
    common.append(
        RadioSetting(
...
def _get_vfo_scan(self, vfoscan):
    ...
    freq_start = _vfo_scan.vhffreq_start / FREQ_STEP_MHZ
    vfoscan.append(
        RadioSetting(
            "vfoscan.vhffreq_start", "Start Frequency",
            RadioSettingValueFloat(108, 600, freq_start, 0.00001, 5)))

    freq_end = _vfo_scan.vhffreq_end / FREQ_STEP_MHZ
    vfoscan.append(
        RadioSetting(
...
def _get_aprs_setting(self, aprs):
    ...
    opts = ["100", "150", "200", "250", "300", "400", "500", "750", "1000"]
    aprs.append(RadioSetting(
        "aprsinfo.tx_delay", "APRS Tx Delay[ms]",
        RadioSettingValueList(opts, current_index=_aprsinfo.tx_delay)))

    opts = ["%s" % x for x in range(3, 16, 1)] + ["Infinite"]
    aprs.append(RadioSetting(
        "aprsinfo.screenhold", "APRS Screen Hold Time[s]",
...
def _get_ana_aprs_setting(self, aprs):
    ...
    aprs.append(
        RadioSetting(
            ...
            RadioSettingValueBoolean(_aprsinfo.anaptttype)))
    aprs.append(
        RadioSetting(
    ...
    callsign_len = 8
    rs = RadioSetting(
        "aprsinfo.call.callsign", "My Callsign",
        RadioSettingValueString(
            0, callsign_len,
            "".join(retevis_ha1g.filter(
                _aprsinfo.call.callsign, retevis_ha1g.NAMECHARSET,
                callsign_len, True)),
            False, retevis_ha1g.NAMECHARSET))
    rs.set_apply_callback(
        _set_char_callback, _aprsinfo.call, "callsign",
        callsign_len, retevis_ha1g.NAMECHARSET)
    aprs.append(rs)

    opts = ["-%d" % x for x in range(0, 16, 1)]
    rs = RadioSetting(
        "aprsinfo.call.ssid", "My SSID",
        RadioSettingValueList(opts, current_index=_aprsinfo.call.ssid))
    rs.set_apply_callback(_set_int_callback, _aprsinfo.call, "ssid")
    aprs.append(rs)
    opts = [
    ...
    for i in range(len(_aprsinfo.ana_chs)):
        ch_freq = _aprsinfo.ana_chs[i].freq / (FREQ_STEP_KHZ)
        rs = RadioSetting(
        ...
        rs.set_apply_callback(_set_freq_callback,
                              _aprsinfo.ana_chs[i], "freq", FREQ_STEP_KHZ)
        rsg.append(rs)
    aprs.append(rsg)
...
def _get_fm_setting(self, fm):
    ...
    for i in range(len(_fminfo)):
        fm_freq = _fminfo[i].freq / FREQ_STEP_MHZ
        rs = RadioSetting(
            "fms.freq_%s" % i, "freq %s" % (i + 1),
            RadioSettingValueFloat(50, 115, fm_freq, 0.00001, 5))
        rs.set_apply_callback(_set_freq_callback,
                              _fminfo[i], "freq", FREQ_STEP_MHZ)
        fm.append(rs)
...
def _get_callsign_setting(rsg, item, name):
    ...
    for i in range(len(item)):
        rs = RadioSetting(
            "%s.callsign_%s" % (name, i), "%s Callsign %s" % (name, i + 1),
            RadioSettingValueString(
                0, callsign_len,
                "".join(retevis_ha1g.filter(
        ...
        rsg.append(rs)
        rs = RadioSetting(
            "%s.ssid_%s" % (name, i), "%s SSID %s" % (name, i + 1),
...
def _set_char_callback(set_item, obj, name: str, charlen: int, charset=None):
    if charset is None:
        ...
        setattr(obj, name, retevis_ha1g.filter(
            set_item.value, retevis_ha1g.NAMECHARSET, 12, True
        ).ljust(charlen, "\x00"))
...
class HA2(retevis_ha1g.HA1G):
    ...
    def get_memory(self, number):
        ...
        channels_len = len(self._memobj.channels)
        if isinstance(number, str):
            mem.extd_number = number
            ch_index = 0 if number == "VFOA" else 1
            mem.number = channels_len + ch_index + 1
            _mem = self._memobj.vfochannels[ch_index]
