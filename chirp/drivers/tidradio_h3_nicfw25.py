...
# V2.5 EEPROM layout (big-endian).
# See nicfw2docs eeprom.md, channelInfo.md, settingsBlock.md.
MEM_FORMAT = """
...
struct {
    ...
    // CHIRP bitwise packs first-declared bitfield at the MSB of the byte (bit
    // 7),
    // last at LSB (bit 0). nicFW wire order is bit0=bandwidth, bit7=busyLock,
    // bits1-2=modulation, bit3=position, bits4-5=pttID, bit6=reversed — so
    // declare MSB→LSB.
    u8 busyLock:1,
       reversed:1,
       pttID:2,
...
struct {
    ...
    u8 micGain;
    u8 txDeviation;
    i8 xtal671_DEFUNCT;  // V2.5: unused; XTAL live at 0x1DFB (eeprom.md)
    u8 battStyle;
    u16 scanRange;
    u16 scanPersist;
    u8 scanResume;
    ...
    u8 dualWatchDelay;
    u8 subToneDeviation;
    u8 filler1967[13];   // 0x1967-0x1973: showXmitCurrent, AGC, RFi Comp, etc.
...
struct {
    ...
    // Radio stores: bit0=txAllowed, bit1=wrap, bits2-4=modulation,
    // bits5-7=bandwidth.
    // Declaring in reverse order makes CHIRP read them correctly (last
    // declared → bit 0).
    u8 bandwidth:3,
       modulation:3,
       wrap:1,
...
struct {
    ...
    u8 persist;
    // CHIRP bitwise assigns fields MSB-first (first declared = highest bit).
    // Radio stores: bits[1:0]=modulation (0=FM,1=AM,2=USB,3=Auto),
    // bits[4:2]=ultrascan(0-7), bits[7:5]=unused.
    // ultrascan declared as 6 bits to cover bits[7:2] (includes the 3 unused
    // upper bits).
    // Declaring in reverse order makes CHIRP map modulation correctly to
    // bits[1:0].
    u8 ultrascan:6,
       modulation:2;
    char label[9];
...
// 0x1C90 group labels (A-O = 15; index 0=Group A .. 14=Group O, 15=unused). 6
...
#seekto 0x1DFB;
struct {
    i8 xtal671;   // Crystal calibration (V2.5; Advanced Menu)
...
# chirp_common.MODES-style labels for FM/AM + narrow.
# EEPROM still stores modulation + bandwidth bit.
NFM = "NFM"
NAM = "NAM"
# Memory editor / validate_memory: NFM/NAM alongside EEPROM modulation names.
VALID_MODES = ["Auto", "FM", NFM, "AM", NAM, "USB"]
BANDWIDTH_LIST = ["Wide", "Narrow"]


def _extra_bandwidth_value_str(mem):
    """Return the Bandwidth extra list value as a string,
    or None if unset/missing."""
...
def _channel_memory_wants_narrow(mem):
    """True if mem should encode narrow bandwidth bit (single source for
    set_memory / mmap patch)."""
    return _chirp_mode_to_fw_channel(mem)[1]


def _apply_channel_bandwidth_bit0_to_mmap(mmap, index, want_narrow):
    """Ensure byte 15 bit0 matches narrow (0=Wide, 1=Narrow);
...
# txPower: 0 = N/T (No Transmit); 1..Max Power = transmit level (Max Power
# = settings VHF/UHF, e.g. 130).
...
# nicFW step: 2.5, 5, 6.25, 12.5, 25, 50 kHz. Stored in radio as Hz.
# CHIRP dropdown shows Hz (2500=2.5 kHz).
...
# Band Plan (nicFW Programmer; verified from live EEPROM, March 2026)
# Modulation: 3 bits (raw 0-7). Raw value IS the list index; no remapping.
#   Confirmed from EEPROM dump: raw 0=Ignore, raw 1=FM, raw 2=AM.
# raw 3-7 unconfirmed; labels are best-effort based on nicFW firmware
# source patterns.
MODULATION_BP_LIST = [
    "Ignore",
    "FM",
...
# Bandwidth: 3 bits (raw 0-7). Raw value IS the list index; no remapping.
#   Confirmed: raw 0=Ignore, 1=Wide, 2=Narrow, 5=FM Tuner.
...
# Scan Preset modulation: 2 bits at bits[1:0]. 0=FM, 1=AM, 2=USB, 3=Auto.
# Verified from live EEPROM dump (March 2026). Different ordering from
# channel modulation (Auto/FM/AM/USB).
MODULATION_SP_LIST = ["FM", "AM", "USB", "Auto"]
# Scan Preset ultrascan level: 3 bits at bits[4:2] of flags byte.
# Displayed as "0"-"7".
...
def _write_block(radio, block_num, data):
    ...
    if ack != bytes([CMD_WRITE_EEPROM]):
        raise errors.RadioError(
            "Radio did not acknowledge write block %d" % block_num
        )
...
def _decode_tone(tone_word):
    ...
    if tone_word & 0x8000:
        ...
        # 9-bit index into ALL_DTCS_CODES (0..511), not the CHIRP
...
def _dtcs_chirp_value_to_firmware_index(value):
    """Map Memory dtcs/rx_dtcs to 9-bit EEPROM index for _encode_tone."""
    if value is None:
        return 0
    codes = chirp_common.ALL_DTCS_CODES
    v = int(value)
...
def _encode_tone(mode, value, polarity=None):
    """Encode tone (mode, value, polarity) as 16-bit big-endian word."""
...
def _get_channel_info(memobj, index):
    """Return channel struct: vfoA (-2), vfoB (-1), or memory[0..197]."""
    if index == -2:
        return memobj.vfoA
    if index == -1:
        return memobj.vfoB
...
def _build_groups_display(memobj):
    """Build per-slot group labels (optional EEPROM text).

    Slot 0 is None; 1-15 are A-O with optional ': label' text.
    """
    display = ["None"]
    for i, letter in enumerate(GROUP_LETTERS):
        try:
            ...
            raw = gl.get_raw()
            lbl = (
                raw.decode("ascii", "replace").rstrip("\x00 \xff").strip()
...
def _channel_to_memory(memobj, number, mem):
    ...
    mem.power = (
        "N/T" if _tx == TXPower_NT else (str(_tx) if 1 <= _tx <= 255 else "1")
    )
    ...
    if raw and len(raw) >= 32:
        ...
        mem.name = (
            name_bytes.decode("ascii", "replace").rstrip("\x00 \xff").strip()
            or ""
        )
    else:
        name_parts = []
    ...
    mod_idx = (
        int(_mem.modulation)
        if int(_mem.modulation) < len(MODULATION_LIST)
        else 1
    )
    ...
    # Groups, bandwidth, busyLock: mem.extra. See chirp_common.Memory.
    ...
    for slot, val in [
        ...
        rs = RadioSetting(
            ...
            RadioSettingValueList(groups_display, groups_display[val]),
        )
        mem.extra.append(rs)
    ...
    mem.extra.append(
        RadioSetting(
            "bandwidth", "Bandwidth", RadioSettingValueList(BANDWIDTH_LIST, bw)
        )
    )
...
def _memory_to_channel(memobj, number, mem):
    ...
    _mem.txPower = (
        ...
        if mem.power == "N/T"
            ...
            int(mem.power)
            if mem.power.isdigit() and 1 <= int(mem.power) <= 255
            else 1
        )
    )
    ...
    _busy_requested = bool(
        mem.extra
        and any(
            e.get_name() == "busyLock" and bool(e.value) for e in mem.extra
        )
    )
    _mem.busyLock = (
        1 if (_busy_requested and mem.duplex not in ("+", "-", "split")) else 0
    )
    ...
    if mem.extra:
        ...
        letters = [
            c.upper()
            for c in mem.comment.strip()
            if c.upper() in GROUP_LETTERS
        ][:4]
        g0 = g1 = g2 = g3 = 0
...
class TH3NicFw25(chirp_common.CloneModeRadio):
    ...
    def get_features(self):
        ...
        # RX span per nicFW Programmer band plan (TX limited by firmware).
        # CHIRP uses lo <= freq < hi; hi = 600_000_001 Hz so 600.0 MHz is
        # included.
        rf.valid_bands = [
            (50_000_000, 600_000_001),
        ]
        # Optional RX sub-ranges (reference; single band above):
        #   (50_000_000, 76_000_000),      # Low VHF / FM
        #   (108_000_000, 136_000_000),  # AM air 8.33k, AM demod
    ...
    def process_mmap(self):
        ...
        if magic != MAGIC_SETTINGS_V25:
            LOG.warning(
                "Settings magic 0x%04X is not V2.5 0xD82F; bad firmware image",
                magic,
            )
    ...
    def validate_memory(self, mem):
        """Check NFM/NAM vs extra bandwidth (RadioDroid / mmap)."""
            ...
            if bw is not None and "Narrow" not in bw:
                msgs.append(
                    chirp_common.ValidationError(
                        "Mode %s requires narrow bandwidth in "
                        "Radio Specific settings; set Bandwidth to Narrow or "
                        "use FM/AM for wideband." % mem.mode))
        return msgs
    ...
    def get_settings(self):
        ...
        basic.append(
            RadioSetting(
                "squelch",
                "Squelch",
                RadioSettingValueInteger(0, 9, int(s.squelch)),
            )
        )
        basic.append(
            RadioSetting(
                "dualWatch",
                "Dual Watch",
                RadioSettingValueBoolean(bool(s.dualWatch)),
            )
        )
        basic.append(
            RadioSetting(
                ...
                "Active VFO",
                RadioSettingValueList(
                    ACTIVEVFO_LIST, ACTIVEVFO_LIST[min(int(s.activeVfo), 1)]
                ),
            )
        )
        step_val = int(s.step)
        step_idx = (
            STEP_VALUES.index(step_val) if step_val in STEP_VALUES else 0
        )
        basic.append(
            RadioSetting(
                "step",
                "Tuning Step (10 Hz)",
                RadioSettingValueList(STEP_LIST, STEP_LIST[step_idx]),
            )
        )
        basic.append(
            RadioSetting(
                "rxSplit",
                "RX Split (10 Hz)",
                RadioSettingValueInteger(0, 65535, int(s.rxSplit)),
            )
        )
        basic.append(
            RadioSetting(
                "txSplit",
                "TX Split (10 Hz)",
                RadioSettingValueInteger(0, 65535, int(s.txSplit)),
            )
        )
        basic.append(
            RadioSetting(
                "pttMode",
                "PTT Mode",
                RadioSettingValueInteger(0, 255, int(s.pttMode)),
            )
        )
        basic.append(
            RadioSetting(
                "txModMeter",
                "TX Modulation Meter",
                RadioSettingValueBoolean(bool(s.txModMeter)),
            )
        )
        basic.append(
            RadioSetting(
                "micGain",
                "Mic Gain",
                RadioSettingValueInteger(0, 31, int(s.micGain)),
            )
        )
        basic.append(
            RadioSetting(
                "txDeviation",
                "TX Deviation",
                RadioSettingValueInteger(0, 255, int(s.txDeviation)),
            )
        )
        basic.append(
            RadioSetting(
                ...
                "Battery Style",
                RadioSettingValueList(
                    BATTSTYLE_LIST, BATTSTYLE_LIST[min(int(s.battStyle), 3)]
                ),
            )
        )
        top.append(basic)
        ...
        scan.append(
            RadioSetting(
                "scanRange",
                "Scan Range (10 Hz)",
                RadioSettingValueInteger(0, 65535, int(s.scanRange)),
            )
        )
        scan.append(
            RadioSetting(
                "scanPersist",
                "Scan Persist",
                RadioSettingValueInteger(0, 65535, int(s.scanPersist)),
            )
        )
        scan.append(
            RadioSetting(
                "scanResume",
                "Scan Resume",
                RadioSettingValueInteger(0, 255, int(s.scanResume)),
            )
        )
        scan.append(
            RadioSetting(
                "ultraScan",
                "Ultra Scan",
                RadioSettingValueInteger(0, 255, int(s.ultraScan)),
            )
        )
        scan.append(
            RadioSetting(
                "scanUpdate",
                "Scan Update",
                RadioSettingValueInteger(0, 255, int(s.scanUpdate)),
            )
        )
        top.append(scan)
        ...
        disp.append(
            RadioSetting(
                "lcdBrightness",
                "LCD Brightness",
                RadioSettingValueInteger(0, 28, int(s.lcdBrightness)),
            )
        )
        disp.append(
            RadioSetting(
                "lcdTimeout",
                "LCD Timeout",
                RadioSettingValueInteger(0, 200, int(s.lcdTimeout)),
            )
        )
        disp.append(
            RadioSetting(
                "breathe",
                "Breathe",
                RadioSettingValueInteger(0, 255, int(s.breathe)),
            )
        )
        disp.append(
            RadioSetting(
                "dimmer",
                "Dimmer",
                RadioSettingValueInteger(0, 255, int(s.dimmer)),
            )
        )
        disp.append(
            RadioSetting(
                "lcdInverted",
                "LCD Inverted",
                RadioSettingValueBoolean(bool(s.lcdInverted)),
            )
        )
        disp.append(
            RadioSetting(
                "repeaterTone",
                "Repeater Tone (Hz)",
                RadioSettingValueInteger(0, 65535, int(s.repeaterTone)),
            )
        )
        disp.append(
            RadioSetting(
                "dtmfDev",
                "DTMF Deviation",
                RadioSettingValueInteger(0, 255, int(s.dtmfDev)),
            )
        )
        disp.append(
            RadioSetting(
                "gamma",
                "Gamma",
                RadioSettingValueInteger(0, 255, int(s.gamma)),
            )
        )
        disp.append(
            RadioSetting(
                "sBarStyle",
                "Signal Bar Style",
                RadioSettingValueInteger(0, 255, int(s.sBarStyle)),
            )
        )
        disp.append(
            RadioSetting(
                "sqNoiseLev",
                "Squelch Noise Level",
                RadioSettingValueInteger(0, 255, int(s.sqNoiseLev)),
            )
        )
        disp.append(
            RadioSetting(
                ...
                RadioSettingValueBoolean(bool(s.sBarAlwaysOn)),
            )
        )
        disp.append(
            RadioSetting(
        ...
        disp.append(
            RadioSetting(
                "ifFreq",
                "IF Freq",
                RadioSettingValueInteger(0, 255, int(s.ifFreq)),
            )
        )
        top.append(disp)
        ...
        vfo_a.append(
            RadioSetting(
                ...
                RadioSettingValueInteger(0, 15, int(s.vfoState_group0)),
            )
        )
        vfo_a.append(
            RadioSetting(
                "vfoState_lastGroup0",
                "Last Group",
                RadioSettingValueInteger(0, 15, int(s.vfoState_lastGroup0)),
            )
        )
        for i in range(16):
            v = min(
                max(int(s.vfoState_groupModeChannels0[i]), 0), 197
            )  # clamp so UI always shows a value (0-197)
            vfo_a.append(
                RadioSetting(
                    "vfoState_groupModeChannels0_%d" % i,
                    "Group Channel %d" % i,
                    RadioSettingValueInteger(0, 197, v),
                )
            )
        vfo_a.append(
            RadioSetting(
                ...
                RadioSettingValueList(
                    OP_MODE_LIST, OP_MODE_LIST[min(int(s.vfoState_mode0), 1)]
                ),
            )
        )
        top.append(vfo_a)
        ...
        vfo_b.append(
            RadioSetting(
                "vfoState_group1",
                "Group (0=channel mode)",
                RadioSettingValueInteger(0, 15, int(s.vfoState_group1)),
            )
        )
        vfo_b.append(
            RadioSetting(
                "vfoState_lastGroup1",
                "Last Group",
                RadioSettingValueInteger(0, 15, int(s.vfoState_lastGroup1)),
            )
        )
        for i in range(16):
            v = min(
                max(int(s.vfoState_groupModeChannels1[i]), 0), 197
            )  # clamp so UI always shows a value (0-197)
            vfo_b.append(
                RadioSetting(
                    "vfoState_groupModeChannels1_%d" % i,
                    "Group Channel %d" % i,
                    RadioSettingValueInteger(0, 197, v),
                )
            )
        vfo_b.append(
            RadioSetting(
                ...
                "Mode (0=VFO, 1=Channel/Group)",
                RadioSettingValueList(
                    OP_MODE_LIST, OP_MODE_LIST[min(int(s.vfoState_mode1), 1)]
                ),
            )
        )
        top.append(vfo_b)
        ...
        misc.append(
            RadioSetting(
                "keyLock",
                "Key Lock",
                RadioSettingValueBoolean(bool(s.keyLock)),
            )
        )
        misc.append(
            RadioSetting(
                "bluetooth",
                "Bluetooth",
                RadioSettingValueBoolean(bool(s.bluetooth)),
            )
        )
        misc.append(
            RadioSetting(
                "powerSave",
                "Power Save",
                RadioSettingValueBoolean(bool(s.powerSave)),
            )
        )
        misc.append(
            RadioSetting(
                "keyTones",
                "Key Tones",
                RadioSettingValueBoolean(bool(s.keyTones)),
            )
        )
        misc.append(
            RadioSetting(
                "ste",
                "STE (Squelch Tail Elim)",
                RadioSettingValueInteger(0, 255, int(s.ste)),
            )
        )
        misc.append(
            RadioSetting(
                ...
                RadioSettingValueList(
                    RFGAIN_LIST,
                    RFGAIN_LIST[min(int(s.rfGain), len(RFGAIN_LIST) - 1)],
                ),
            )
        )
        misc.append(
            RadioSetting(
                "lastFmtFreq",
                "Last FM Tuner Freq",
                RadioSettingValueInteger(0, 0xFFFFFFFF, int(s.lastFmtFreq)),
            )
        )
        misc.append(
            RadioSetting(
        ...
        misc.append(
            RadioSetting(
                "voxTail",
                "VOX Tail",
                RadioSettingValueInteger(0, 65535, int(s.voxTail)),
            )
        )
        misc.append(
            RadioSetting(
                "txTimeout",
                "TX Timeout (s)",
                RadioSettingValueInteger(0, 255, int(s.txTimeout)),
            )
        )
        misc.append(
            RadioSetting(
                "dtmfSpeed",
                "DTMF Speed",
                RadioSettingValueInteger(0, 255, int(s.dtmfSpeed)),
            )
        )
        misc.append(
            RadioSetting(
                "noiseGate",
                "Noise Gate",
                RadioSettingValueInteger(0, 255, int(s.noiseGate)),
            )
        )
        misc.append(
            RadioSetting(
                "asl", "ASL", RadioSettingValueInteger(0, 255, int(s.asl))
            )
        )
        misc.append(
            RadioSetting(
                "disableFmt",
                "Disable FM Tuner",
                RadioSettingValueBoolean(bool(s.disableFmt)),
            )
        )
        misc.append(
            RadioSetting(
                "dualWatchDelay",
                "Dual Watch Delay",
                RadioSettingValueInteger(0, 255, int(s.dualWatchDelay)),
            )
        )
        misc.append(
            RadioSetting(
                "subToneDeviation",
                "Sub Tone Deviation",
                RadioSettingValueInteger(0, 127, int(s.subToneDeviation)),
            )
        )
        misc.append(
            RadioSetting(
                "amAgcFix",
                "AM AGC Fix",
                RadioSettingValueBoolean(bool(s.amAgcFix)),
            )
        )
        top.append(misc)
        ...
        sec.append(
            RadioSetting(
                "pin", "PIN", RadioSettingValueInteger(0, 65535, int(s.pin))
            )
        )
        sec.append(
            RadioSetting(
                "pinAction",
                "PIN Action",
                RadioSettingValueInteger(0, 255, int(s.pinAction)),
            )
        )
        sec.append(
            RadioSetting(
                "lockedVfo",
                "Locked VFO",
                RadioSettingValueInteger(0, 255, int(s.lockedVfo)),
            )
        )
        sec.append(
            RadioSetting(
                "vfoLockActive",
                "VFO Lock Active",
                RadioSettingValueBoolean(bool(s.vfoLockActive)),
            )
        )
        top.append(sec)
        ...
        for i in range(20):
            ...
            plan.append(
                RadioSetting(
                    ...
                    RadioSettingValueList(
                        MAXPOWER_BP_LIST,
                        MAXPOWER_BP_LIST[
                            min(int(bp.maxPower), len(MAXPOWER_BP_LIST) - 1)
                        ],
                    ),
                )
            )
            ...
            plan.append(
                RadioSetting(
                    "bandPlan_%d_modulation" % i,
                    "Modulation",
                    RadioSettingValueList(
                        MODULATION_BP_LIST, MODULATION_BP_LIST[_mod_idx]
                    ),
                )
            )
            plan.append(
                RadioSetting(
                    "bandPlan_%d_bandwidth" % i,
                    "Bandwidth",
                    RadioSettingValueList(
                        BANDWIDTH_BP_LIST, BANDWIDTH_BP_LIST[_bw_idx]
                    ),
                )
            )
        ...
        # Scan Presets at 0x1B00; empty entry when startFreq == 0
        # Modulation: 0=FM, 1=AM, 2=USB, 3=Auto (EEPROM Mar 2026).
        # Ultrascan: 3 bits at bits[4:2] of flags byte; range 0-7.
        # Range is 10 kHz units (End = Start + Range).
        ...
        for i in range(20):
            ...
            # Empty slots (startFreq==0): Programmer hides end/step fields
            # entries even though the EEPROM bytes may contain non-zero default
            # values.
            if start_raw == 0:
                end_mhz = 0.0
                step_khz = 0.0
            ...
            try:
                ...
                # is null terminator)
                label_bytes = (
                    bytes(raw[11:19]) if raw and len(raw) >= 19 else b""
                )
                label_str = (
            ...
            preset = RadioSettingGroup(
                "scanPreset_%d" % i, "Preset %d" % (i + 1)
            )
            preset.append(
                RadioSetting(
                    "scanPreset_%d_startFreq" % i,
                    "Start (MHz)",
                    RadioSettingValueFloat(0, 1500, start_mhz, 0.00001),
                )
            )
            ...
            preset.append(
                RadioSetting(
                    "scanPreset_%d_modulation" % i,
                    "Modulation",
                    RadioSettingValueList(
                        MODULATION_SP_LIST, MODULATION_SP_LIST[_mod_raw]
                    ),
                )
            )
            preset.append(
                RadioSetting(
                    "scanPreset_%d_ultrascan" % i,
                    "Ultrascan (0-7)",
                    RadioSettingValueList(
                        ULTRASCAN_SP_LIST, ULTRASCAN_SP_LIST[_ultrascan]
                    ),
                )
            )
            preset.append(
                RadioSetting(
                    "scanPreset_%d_label" % i,
                    "Label (8 chars)",
            ...
            try:
                ...
                label_str = "".join(
                    ...
                    and 32 <= ord(c) < 127
                        ...
                        chr(c) if isinstance(c, int) and 32 <= c < 127 else ""
                    )
                    for c in gl.label
                ).rstrip()
            gl_grp.append(
                RadioSetting(
                    "groupLabel_%d" % i,
                    "Group %s" % GROUP_LETTERS[i],
                    RadioSettingValueString(0, 6, label_str),
                )
            )
        top.append(gl_grp)
        ...
        adv.append(
            RadioSetting(
                ...
                RadioSettingValueInteger(0, 255, int(cal.maxPowerWattsUHF)),
            )
        )
        adv.append(
            RadioSetting(
                ...
                RadioSettingValueInteger(0, 255, int(cal.maxPowerSettingUHF)),
            )
        )
        adv.append(
            RadioSetting(
                ...
                RadioSettingValueInteger(0, 255, int(cal.maxPowerWattsVHF)),
            )
        )
        adv.append(
            RadioSetting(
                ...
                RadioSettingValueInteger(0, 255, int(cal.maxPowerSettingVHF)),
            )
        )
        top.append(adv)
    ...
    def set_settings(self, ui):
        def apply_el(element):
            ...
            val = (
                ...
                if hasattr(element.value, "get_value")
                else element.value
            )
    ...
    def _apply_one_setting(self, name, val):
        """Apply one (name, value) to the settings struct.

        Used from set_settings and apply_setting_to_settings.
        """
        s = self._memobj.settings
        if name == "squelch":
            ...
            s.activeVfo = (
                ACTIVEVFO_LIST.index(val) if val in ACTIVEVFO_LIST else 0
            )
        elif name == "step":
            ...
            s.battStyle = (
                BATTSTYLE_LIST.index(val) if val in BATTSTYLE_LIST else 0
            )
        elif name == "scanRange":
            ...
            s.toneMonitor = (
                TONEMONITOR_LIST.index(val) if val in TONEMONITOR_LIST else 0
            )
            ...
            s.afFilters = (
                min(AF_FILTERS_LIST.index(val), 255)
            ...
            s.vfoState_mode0 = (
                OP_MODE_LIST.index(val) if val in OP_MODE_LIST else 0
            )
                ...
                try:
                    ...
                    if 0 <= idx < 20:
                        ...
                        if field == "startFreq":
                            ...
                            bp.modulation = (
                                MODULATION_BP_LIST.index(val)
                                if val in MODULATION_BP_LIST
                                else 0
                            )
                        elif field == "bandwidth":
                            bp.bandwidth = (
                                BANDWIDTH_BP_LIST.index(val)
    ...
    def apply_setting(self, name, val):
        """Apply one (name, value) to the memory struct (Android path).

        Use when tree updates do not persist to structs.
        """
        self._apply_one_setting(name, val)

    def apply_setting_to_settings(self, name, val):
        """Deprecated: call apply_setting instead."""
