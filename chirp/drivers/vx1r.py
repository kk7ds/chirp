...
class VX1Radio(yaesu_clone.YaesuCloneModeRadio):
    ...
    def sync_out(self):
        ...
        try:
            ...
            # Write the data in chunks for progress updates
            block_size = 32  # Same as _block_size
            written = 0
            for i in range(0, len(mmap), block_size):
            ...
            # Consume the echo first
            time.sleep(0.1)  # Give time for echo
            echo = self.pipe.read(len(mmap))
            if len(echo) != len(mmap):
                raise Exception("Failed to read complete echo")

            # Then read the ACK
            time.sleep(0.1)  # Give time for ACK
            ack = self.pipe.read(1)
            if len(ack) != 1 or ack[0] != self.CMD_ACK:
                raise Exception("Radio did not ack block")
