...
# The 52 Channel Group 1 (CG1) 'regular' channels
#     (allowing duplex and complete tone settings)
# The 8 band VFOs
# Channel Group 1 (CG1) (except the regular channels, above):
...
# CHIRP requires there to be some 'regular' channels for unit testing.
# Therefore I chose CG1 1-52 to be these regular channels at the top.
# This is slightly out of order than I'd otherwise choose for the UI.
...
# Regular memories 1-52: CG1-1 through CG1-52
# Special channels: VFOs, CG1 home, CG1 L/U, CG2, BC Band, Smart Search
VFO_NAMES = ["VFO-BCBAND", "VFO-FM", "VFO-AIR", "VFO-V-HAM",
             "VFO-VHF-TV", "VFO-ACT1", "VFO-U-HAM", "VFO-UHF-TV", "VFO-ACT2"]
SMART_SEARCH_NAMES = ["SS-%d" % (i + 1) for i in range(0, 31)]
BC_NAMES = ["BC-%d" % (i + 1) for i in range(0, 9)]
CG2_HOME_NAMES = ["CG2-H-FM", "CG2-H-AIR", "CG2-H-V-HAM", "CG2-H-VHF-TV",
                  "CG2-H-ACT1", "CG2-H-U-HAM", "CG2-H-UHF-TV", "CG2-H-ACT2"]
CG1_HOME_NAMES = ["CG1-H-FM", "CG1-H-AIR", "CG1-H-V-HAM", "CG1-H-VHF-TV",
                  "CG1-H-ACT1", "CG1-H-U-HAM", "CG1-H-UHF-TV", "CG1-H-ACT2"]
SPECIALS = (
    VFO_NAMES +
    CG1_HOME_NAMES +
    ["CG1-%s%d" % (c, i + 1)
     for i in range(0, 10) for c in ('L', 'U')] +
    CG2_HOME_NAMES +
...
class VX1Checksum(yaesu_clone.YaesuChecksum):
    """VX-1R Checksum at 0x12d1: (0xe0 - sum) & 0xff,
    +0x03 if high byte >= 0x80"""
...
class VX1Checksum2(yaesu_clone.YaesuChecksum):
    """VX-1R Checksum at 0x00a0: (sum + 0x20) & 0xff,
    -0x03 if sum1 high byte >= 0x80
    Calculated first with both 0x00a0 and 0x12d1 zeroed."""

    def get_calculated(self, mmap):
        mmap = self._asbytes(mmap)
        # Calculate sum1 with both checksums zeroed to determine adjustment
        sum1 = sum(mmap[i][0] for i in range(
            0x0000, 0x12d1) if i not in (0x00a0, 0x12d1))
        cs = 0
        for i in range(self._start, self._stop + 1):
            cs += mmap[i][0]
        result = (cs + 0x20) & 0xff
...
class VX1Radio(yaesu_clone.YaesuCloneModeRadio):
    ...
    def _get_extra(self, _mem, mem):
        ...
        # Skip BC VFO (53) and BC band (260-269) - no extra settings
        if mem.number == 53 or (260 <= mem.number <= 269):
            return
    ...
    def _update_band_mask(self, mem_idx, band_idx, value, is_cg2=False):
        ...
        """
        # Adjust index: subtract 8 for regular channels (skip home channels in
        # calculation)
    ...
    def get_memory(self, number):
        ...
        if isinstance(number, str):
            ...
            # Parse CG1 home channels
                ...
                number = 62 + CG1_HOME_NAMES.index(number)
                mem.extd_number = CG1_HOME_NAMES[number - 62]
            # Parse "CG1-L1" through "CG1-L10" or "CG1-U1" through "CG1-U10"
                ...
                if 1 <= pair <= 10:
                    ...
                    mem.extd_number = "CG1-%s%d" % (
                        'U' if is_upper else 'L', pair)
                else:
                    raise errors.InvalidMemoryLocation(
                        "Invalid CG1 band limit: %s" %
                        number)
            ...
            elif (number.startswith("CG2-") and
                  not number[4] in ['L', 'U', 'H']):
                ch_num = int(number[4:])
                if 1 <= ch_num <= 142:
                    number = 97 + ch_num
            ...
            # Parse "CG2-L1" through "CG2-L10" or "CG2-U1" through "CG2-U10"
                ...
                if 1 <= pair <= 10:
                    ...
                    mem.extd_number = "CG2-%s%d" % (
                        'U' if is_upper else 'L', pair)
                else:
                    raise errors.InvalidMemoryLocation(
                        "Invalid CG2 band limit: %s" % number)
            # Parse BC home
            elif number == "BC-H":
                number = 260
            ...
            # Parse smart search channels
                ...
                if 1 <= ss_num <= 31:
                    ...
                    raise errors.InvalidMemoryLocation(
                        "Invalid smart search: %s" % number)
            else:
                raise errors.InvalidMemoryLocation(
                    "Invalid memory: %s" % number)
        ...
        # Memory layout:
        # 1-52: CG1-1 through CG1-52 (regular memories)
        # 53-61: VFOs (special)
        # 62-69: CG1 home channels (special)
        # 70-89: CG1 band limits (special)
        # 90-97: CG2 home (special)
        # 98-239: CG2-1 through CG2-142 (special)
        # 240-259: CG2 band limits (special)
        # 260: BC home (special)
        # 261-269: BC-1 through BC-9 (special)
        # 270-300: SS-1 through SS-31 (special)

        if number <= 52:
            # Regular memories 1-52: CG1-1 through CG1-52
            # Map to CG1 memory indices 8-59 (after the 8 home channels)
            cg1_idx = number + 7
            ...
            has_offset = True
            if not hasattr(mem, 'extd_number'):
                mem.extd_number = CG1_HOME_NAMES[cg1_idx]
        elif number <= 89:
            # CG1 band limits (no shift)
            bl_idx = number - 70
            cg1_idx = 60 + bl_idx
            _mem = self._memobj.cg1_memory[cg1_idx]
            has_offset = True
        ...
        if number >= 260 and number <= 269:  # BC band (including home)
            freq_mhz = 0.5 + (_mem.freq[0] * (1.2 / 254.0))
            mem.freq = int(freq_mhz * 1000000)
            # BC band has 2-byte name, _decode_name handles it
    ...
    def set_memory(self, mem):
        # Validate and apply restrictions before setting
        self.validate_memory(mem)

        number = mem.number

        # String input already converted by get_memory, but handle if called
        # directly
        if isinstance(number, str):
            # Reuse parsing from get_memory - this shouldn't normally happen
            raise errors.InvalidMemoryLocation(
                "set_memory received string: %s" % number)
        ...
        if mem.empty:
            # Set only first byte to 0xff (radio behavior - leaves rest of data
            # intact)
            if number >= 1 and number <= 52:
                # CG1 regular memories (1-52 map to cg1_memory[8-59])
                cg1_idx = number + 7
                offset = 0x0bd1 + cg1_idx * 16
            elif number >= 54 and number <= 61:
                # VFOs (54-61, skip 53 which is BC VFO)
                vfo_idx = number - 53 - 1
                offset = 0x0321 + vfo_idx * 16
            elif number == 53:
                # BC VFO
                offset = 0x0311
        ...
        if number == 53:
            ...
            # Always write 0x40 (4.0 kHz in BCD) regardless of UI value
            _mem.freq[1] = 0x40
            # BC VFO has 2-byte name field
            name_bytes = _encode_name(
                mem.name[:2] if mem.name else "", length=2)
        ...
        if has_offset:
            ...
            # For split mode, offset contains TX frequency; otherwise it's an
            # offset
        ...
        # Update band mask based on frequency (must be last, after all fields
        # set)
        if number >= 1 and number <= 52:
            # Regular CG1 memories (1-52 map to CG1 indices 8-59)
            mem_idx = number + 7
            band_idx = self._get_band_index(mem.freq)
            if band_idx is not None:
                self._update_band_mask(mem_idx, band_idx, True, is_cg2=False)
        elif number >= 62 and number <= 89:
            # CG1 home and band limits (62-69 map to indices 0-7, 70-89 map to
            # indices 60-79)
            if number <= 69:
                mem_idx = number - 62
            else:
                mem_idx = 60 + (number - 70)
            band_idx = self._get_band_index(mem.freq)
            if band_idx is not None:
                self._update_band_mask(mem_idx, band_idx, True, is_cg2=False)
        elif number >= 90 and number <= 259:
    ...
    def validate_memory(self, mem):
        ...
        # BC band VFO (53) and BC band (260-269) only support AM mode
        if mem.number == 53 or (260 <= mem.number <= 269):
            if mem.mode != "AM":
                mem.mode = "AM"
        ...
        # CG2 memories (90-259), BC band VFO (53), and BC band (260-269) don't
        # support offset
        if mem.number == 53 or (90 <= mem.number <= 269):
            if mem.duplex:
                mem.duplex = ""
                msgs.append(chirp_common.ValidationWarning(
    ...
    def sync_in(self):
        ...
        if not self.match_model(self._mmap.get_packed(), None):
            raise errors.RadioError("Invalid data received from radio")
    ...
    def match_model(cls, filedata, filename):
        ...
        # Check bits 6-7 are set at key locations
        if (filedata[0x0] & 0xc0) != 0xc0:
            return False
        if (filedata[0x1] & 0xc0) != 0xc0:
            return False
        if (filedata[0x300] & 0xc0) != 0xc0:
            return False
        if (filedata[0x301] & 0xc0) != 0xc0:
            return False

        # Check for "YAESU" string at 0xE6
        if filedata[0xE6:0xEB] != b"YAESU":
            return False
    ...
    def get_settings(self):
        ...
        rs.set_doc("Must match radio's current active mode for clone to be "
                   "accepted (hex format)")
        basic.append(rs)

        rs = RadioSetting(
            "hardware_default_mode", "Hardware Default Mode",
            RadioSettingValueString(
        ...
        rs = RadioSetting(
            "config1.bclo", "Busy Channel Lockout",
        ...
        rs = RadioSetting(
            "config4.dialm", "Dial Mode",
            RadioSettingValueBoolean(_settings.config4.dialm))
        basic.append(rs)

        rs = RadioSetting(
            "config5.grp", "Channel Group",
            RadioSettingValueList(
                ["Group 1", "Group 2"],
        ...
        rs = RadioSetting(
            "config2.rxsave", "RX Battery Save",
            RadioSettingValueList(
