...
HAM_FREQS = [162.55000, 162.40000, 162.47500, 162.42500, 162.45000,
             162.50000, 162.52500, 161.65000, 161.77500, 161.75000]
...
class TDH8(chirp_common.CloneModeRadio):
    ...
    def set_memory(self, mem):
        ...
        if self._gmrs and MODE_RADIO == "P31184":
            ...
            if ((mem.freq >= 136000000 and mem.freq <= 174000000) or (mem.freq >= 400000000 and mem.freq <= 520000000)):
                _mem.rxfreq = _mem.txfreq = mem.freq / 10
            else:
                _mem.offset = 0
                msg = ("The frequency must be between 136.00000-174.00000 or 420.00000-450.00000")
                raise InvalidValueError(msg)
                ...
                try:
                    ...
                    # B channel
                        ...
                        if '.' in str(element.value):
                            val = str(element.value).replace(' ', '')
                            if len(val[val.index(".") + 1:]) >= 1 and int(val[val.index(".") + 1:]) != 0:
                                val = '00' + str(element.value).replace('.', '')
                            else:
                                val = '0' + str(element.value).replace('.', '')
                            val = val.ljust(8, '0')
                            lenth_val = 0
