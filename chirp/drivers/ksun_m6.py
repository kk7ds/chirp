...
# Copyright 2026 Auto-detection support for V1/V2 hardware variants
#
...
# =============================================================================
# V1 Memory Format (Pre-2024 hardware: 80 channels, 4800 baud)
# =============================================================================
...
# =============================================================================
# V2 Memory Format (2024+ hardware: 200 channels, 38400 baud)
# =============================================================================
...
# Additional list constants for V2
...
def _checksum_v1(data):
    """Calculate checksum for V1 protocol"""
    cs = 2
    for byte in data:
        cs += byte
    return cs % 256
...
def enter_programming_mode_v1(radio):
    """Enter programming mode for V1 hardware"""
    serial = radio.pipe

    cmd = b"\x32\x31\x05\x10"
    req = cmd + bytes([_checksum_v1(cmd)])

    # Try 3 times to enter programming mode
    ...
    raise errors.RadioError(
        "Radio refused to enter programming mode after 3 attempts")


def exit_programming_mode_v1(radio):
    """Exit programming mode for V1 hardware"""
    serial = radio.pipe
...
def enter_programming_mode_v2(radio):
    """Enter programming mode for V2 hardware"""
    serial = radio.pipe

    # Command for entering programming mode
    cmd = b"\x32\x31\x05\x10"
    req = cmd + bytes([_checksum_v2(cmd)])  # Should produce 0xCE (206)
...
def exit_programming_mode_v2(radio):
    """Exit programming mode for V2 hardware"""
    serial = radio.pipe

    # Command for exiting programming mode
    cmd = b"\x32\x31\x05\xee"
    req = cmd + bytes([_checksum_v2(cmd)])  # Should produce 0xAC (172)
...
def _read_block_v1(radio, block_addr):
    """Read a memory block from V1 radio"""
    serial = radio.pipe
...
def _write_block_v1(radio, block_addr, block_data):
    """Write a memory block to V1 radio"""
    serial = radio.pipe
...
def _read_block_v2(radio, block_addr):
    """Read a memory block from V2 radio"""
    serial = radio.pipe

    # Read command: 'R' + address
    cmd = struct.pack(">cH", b"R", block_addr)
    req = cmd + bytes([_checksum_v2(cmd)])
...
def _write_block_v2(radio, block_addr, block_data):
    """Write a memory block to V2 radio"""
    serial = radio.pipe
...
def verify_model_v1(radio):
    """Verify V1 radio model"""
    ...
    for _ in range(3):
        try:
            _read_block_v1(radio, radio.START_ADDR)
            return
        except Exception:
            pass
...
def verify_model_v2(radio):
    """Verify V2 radio model"""
    ...
    for attempt in range(5):
        try:
            ...
            block = _read_block_v2(radio, radio.START_ADDR)
...
def do_download_v1(radio):
    """Download memory from V1 radio"""
    status = chirp_common.Status()
    status.msg = "Cloning from radio"
    status.max = radio._memsize

    verify_model_v1(radio)
    ...
    for addr in range(radio.START_ADDR,
        ...
        block = _read_block_v1(radio, addr)
        data += block

    return memmap.MemoryMapBytes(data)


def do_upload_v1(radio):
    """Upload memory to V1 radio"""
    verify_model_v1(radio)
    enter_programming_mode_v1(radio)
    ...
    for addr in range(0, radio._memsize, radio.BLOCK_SIZE):
        ...
        _write_block_v1(radio, radio.START_ADDR + addr, block)

    exit_programming_mode_v1(radio)


def do_download_v2(radio):
    """Download memory from V2 radio"""
    # Enter programming mode first
    enter_programming_mode_v2(radio)
    ...
    try:
        for addr in range(radio.START_ADDR,
            ...
            block = _read_block_v2(radio, addr)
        ...
        try:
            exit_programming_mode_v2(radio)
        except Exception:
...
def do_upload_v2(radio):
    """Upload memory to V2 radio"""
    # Enter programming mode first
    enter_programming_mode_v2(radio)

    # Verify we're talking to the correct radio model
    verify_model_v2(radio)
    ...
    try:
        for addr in range(0, radio._memsize, radio.BLOCK_SIZE):
            ...
            block = mmap[addr:addr + radio.BLOCK_SIZE]
            _write_block_v2(radio, radio.START_ADDR + addr, block)
    finally:
...
# =============================================================================
# V1 Tone Encoding/Decoding
# =============================================================================


def mem_to_triplet_v1(mem_tone, mem_code):
    """Decode V1 tone format to CHIRP triplet"""
...
def triplet_to_mem_v1(tone):
    """Encode CHIRP triplet to V1 tone format"""
...
# =============================================================================
# V2 Tone Encoding/Decoding
# =============================================================================


def decode_tone_v2(tone_value):
    """Decode V2 tone format (16-bit little-endian)
...
def encode_tone_v2(mode, value, polarity):
    """Encode tone to V2 format (16-bit little-endian)
...
# =============================================================================
# Main Radio Class (Original V1 hardware: 80 channels, 4800 baud)
# =============================================================================


@directory.register
class KSunM6Radio(chirp_common.CloneModeRadio):
    """KSUN M6 - Pre-2024 hardware (80 channels, 4800 baud)

    This is the original hardware revision with:
    - 80 memory channels
    - 4800 baud communication
    - Packed 40-bit frequency format
    - No channel names

    Auto-detects V2 hardware and redirects to KSunM6V2Radio if found.
    """
    VENDOR = "KSUN"
    MODEL = "M6"
    BAUD_RATE = 4800
    ...
    def detect_from_serial(cls, pipe):
        """Auto-detect V1 vs V2 hardware variant

        Try V1 first (safer for hardware), then V2 if V1 fails.
        """
        # Try V1 first (this class)
        pipe.baudrate = cls.BAUD_RATE
        radio = cls(pipe)
        try:
            enter_programming_mode_v1(radio)
            verify_model_v1(radio)
            LOG.info('Detected KSUN M6 V1')
            return cls
        except errors.RadioError:
            LOG.debug('No response from radio with V1 protocol')
            try:
                exit_programming_mode_v1(radio)
            except Exception:
                pass
        ...
        raise errors.RadioError(
            'Could not detect KSUN M6 variant. '
            'Please verify: (1) Radio is powered on, '
            '(2) USB cable is properly connected, '
            '(3) No other software is accessing the radio')

    def get_features(self):
        """Return RadioFeatures object describing radio capabilities"""
    ...
    def sync_in(self):
        """Download from radio"""
        self._mmap = do_download_v1(self)
        self.process_mmap()
    ...
    def process_mmap(self):
        """Parse memory map into structured objects"""
        self._memobj = bitwise.parse(MEM_FORMAT_V1, self._mmap)

    def get_raw_memory(self, number):
        """Get raw memory representation for debugging"""
        return repr(self._memobj.memory[number-1])

    def get_memory(self, number):
        """Extract high-level memory object from memory map"""
        ...
        if _mem.freq.get_raw() == bytes([255] * 5):
            ...
            chirp_common.split_tone_decode(
                mem,
                mem_to_triplet_v1(_mem.tx_tone, _mem.tx_code),
                mem_to_triplet_v1(_mem.rx_tone, _mem.rx_code))
    ...
    def set_memory(self, mem):
        """Store high-level memory object to memory map"""
        ...
        if mem.empty:
            ...
            _mem.tx_tone, _mem.tx_code = triplet_to_mem_v1(tx_tone)
            _mem.rx_tone, _mem.rx_code = triplet_to_mem_v1(rx_tone)
...
# =============================================================================
# V2 Radio Class (2024+ hardware: 200 channels, 38400 baud)
# =============================================================================


@directory.register
@directory.detected_by(KSunM6Radio)
class KSunM6V2Radio(chirp_common.CloneModeRadio):
    """KSUN M6 V2 - 2024+ hardware (200 channels, 38400 baud)"""
    VENDOR = "KSUN"
    MODEL = "M6"
    VARIANT = "V2"
    BAUD_RATE = 38400
    ...
    def sync_in(self):
        """Download from radio"""
        self._mmap = do_download_v2(self)
        self.process_mmap()
    ...
    def process_mmap(self):
        """Parse memory map into structured objects"""
        self._memobj = bitwise.parse(MEM_FORMAT_V2, self._mmap)
    ...
    def get_memory(self, number):
        ...
        if _mem.rx_freq == 0xFFFFFFFF:
            ...
            txtone = decode_tone_v2(_mem.tx_tone)
            rxtone = decode_tone_v2(_mem.rx_tone)
    ...
    def set_memory(self, mem):
        ...
        if mem.empty:
            ...
            _mem.tx_tone = encode_tone_v2(*txtone)
            _mem.rx_tone = encode_tone_v2(*rxtone)
