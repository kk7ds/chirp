...
MUTE_MODE_MAP = [('QT',      0b01),
                 ('QT*DTMF', 0b10),
                 ('QT+DTMF', 0b11)]
STEPS = [5.0, 6.25, 10.0, 12.5, 20.0, 25.0, 30.0, 50.0, 100.0]
...
# Support for the Wouxun KG-UVR5 radio
...
@directory.register
class KGUVR5Radio(chirp_common.CloneModeRadio,
    ...
    def process_mmap(self):
        ...
        #Debug: print(util.hexprint(self._mmap))
    ...
    def get_features(self):
        ...
        rf.valid_bands = [(136000000, 174000000),  # supports 2m
                          (200000000, 310999999)]  # supports 200-310mhz
    ...
    def _get_settings(self):
        ...
        limits_grp = RadioSettingGroup("limits_grp", "Frequency ranges")
        oem_grp = RadioSettingGroup("oem_grp", "OEM Info")

        group = RadioSettings(cfg_grp, ui_grp, vfoa_grp, vfob_grp, scn_grp,
                              rpt_grp, rmt_grp, fmp_grp, limits_grp, oem_grp)
        ...
        rs = RadioSetting("step_a", "Step (kHz)",
                          RadioSettingValueList(
                              STEP_LIST, current_index=_settings.step_a - 1))
        vfoa_grp.append(rs)
        ...
        rs = RadioSetting("step_b", "Step (kHz)",
                          RadioSettingValueList(
                              STEP_LIST, current_index=_settings.step_b - 1))
        vfob_grp.append(rs)
    ...
    def set_settings(self, settings):
        for group in settings:
            for element in group:
                ...
                # Frequency limits
                #
                if group.get_name() == 'limits_grp':
                    if name == 'vhf_rx_start':
                        value = _limit_encode(element[0].get_value())
                        setattr(self._memobj.limits, setting, value)
                        continue
                    elif name == 'vhf_rx_stop':
                        value = _limit_encode(element[0].get_value())
                        setattr(self._memobj.limits, setting, value)
                        continue
                    elif name == 'vhf_tx_start':
                        value = _limit_encode(element[0].get_value())
                        setattr(self._memobj.limits, setting, value)
                        continue
                    elif name == 'vhf_tx_stop':
                        value = _limit_encode(element[0].get_value())
                        setattr(self._memobj.limits, setting, value)
                        continue
                    elif name == 'uhf_rx_start':
                        value = _limit_encode(element[0].get_value())
                        setattr(self._memobj.limits, setting, value)
                        continue
                    elif name == 'uhf_rx_stop':
                        value = _limit_encode(element[0].get_value())
                        setattr(self._memobj.limits, setting, value)
                        continue
                    elif name == 'uhf_tx_start':
                        value = _limit_encode(element[0].get_value())
                        setattr(self._memobj.limits, setting, value)
                        continue
                    elif name == 'uhf_tx_stop':
                        value = _limit_encode(element[0].get_value())
                        setattr(self._memobj.limits, setting, value)
                        continue
