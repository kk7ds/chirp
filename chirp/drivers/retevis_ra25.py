...
SUPPORTED_MODEL_VERSIONS = {b'RA_25UV': [b'V200']}
...
MEMORY_REGIONS = [
    ...
    (0x0062000, 100, 32, 'W', 'memory', 'Memory Chan. 401-500'),
...
def get_ident(serial):
    try:
        bs = send_cmd(serial, b'\x02', 16)
        model, mode, ver = struct.unpack('>x7sB4s3x', bs)
        LOG.info(f'Model {model} Ver {ver} Band Mode {mode}')
        if ver in SUPPORTED_MODEL_VERSIONS[model]:
        ...
        for mr in mrs:
            ...
            if struc == "memory":
                if base == 0x42000:
                    ...
                    struc_data = struc_data[400 * blk_sz: 500 * blk_sz]
...
class RA25UVRadio(chirp_common.CloneModeRadio, chirp_common.ExperimentalRadio):
    ...
    def get_offset(self, _mem, mem):
        # gets offset from _mem for ui
        n = bitwise.bcd_to_int(_mem.tx_offset)
        mem.offset = n * 10

    def set_offset__mem(self, _mem, mem):
        # sets _mem from ui edit
        val = mem.offset/10
        bitwise.int_to_bcd(_mem.tx_offset, val)
