...
def do_download(radio):
    ...
    # _enter_programming_mode() done by detect_from_serial()
        ...
        for magic_list, rclass in magics.items():
            for magic in magic_list:
                try:
                    ...
                    LOG.error('Radio did NOT accept Programming mode '
                              'using magic %s' % magic)
                    continue
    ...
    def sync_out(self):
        ...
        for magic in self._magic_list:
            ...
            try:
                ...
                LOG.warning('.img file too short, can''t update timestamp')
            ...
            try:
                ...
                LOG.info('Radio did not accept magic %s' % magic)
                pass
            except Exception:
                # If anything unexpected happens, make sure we raise
                # a RadioError and log the problem
...
class TDM11_16(TDM11_22):
    ...
    MODEL = 'TD-M11'
    VARIANT = '16 PMR'
    VALID_BANDS = [(446006250, 446193751)]  # PMR freqs
    _power_levels = [
        chirp_common.PowerLevel('Low', watts=0.50),
        chirp_common.PowerLevel('High', watts=2.00)
    ]
    ...
    def get_features(self):
        ...
        rf.valid_modes = ['NFM']  # PMR only has 12.5kHz spacing
        rf.has_offset = False
        rf.valid_duplexes = ['']  # PMR is simplex only
        return rf
    ...
    def get_memory(self, number):
        ...
        _mem = self._memobj.memory[mem.number - 1]
        # force for PMR...
        _mem.txfreq = _mem.rxfreq  # PMR is simplex ony
        mem.mode = 'NFM'
        _mem.narrow = 0b1
        mem.power = self._power_levels[0]
        _mem.txpower = 0b0  # PMR low power only
        mem.duplex = ''
        mem.offset = 0
        # no edits...
        mem.immutable = ['mode', 'power', 'duplex', 'offset']
