...
def handshake(self, serial):
    ...
    for num in range(15):
        flag, databytes = handle_connect_ver(
            get_handshake_bytes(self.MODEL + " "), serial, self.read_page_len)
        time.sleep(0.05)
        if flag:
            break
...
def handle_connect_ver(
    send_byte: bytes, serial_conn, chunk_size=1050, max_retries=5
) -> tuple[bool, bytes]:
    ...
    # Sometimes, because of cable/port instability,
    # one packet is received in fragments.
    # Looping 5 times helps ensure we don't miss data.
    ...
    while page_index < page_count:
        ...
        for attempt in range(max_retries):
            flag, new_data_bytes = handle_connect_ver(
                Get_ReadItemBytes(item, page_index),
                serial, self.read_page_len)
            if (not flag
...
def write_item_current_page_bytes(self, serial, item_Bytes: bytes,
    ...
    max_retries = 5
    retry_delay = 0.05
    page_count = math.ceil(len(item_Bytes) / self.write_page_len)
    if page_count == 0:
        return True
    for i in range(0, page_count):
        for attempt in range(max_retries):
            current_page_bytes = get_write_item_send_bytes(
                item, i, page_count,
                item_Bytes[
                    i * self.write_page_len: (i + 1) * self.write_page_len
                    ])
            flag, new_data_bytes = handle_connect_ver(
                current_page_bytes,
                serial, self.read_page_len)
