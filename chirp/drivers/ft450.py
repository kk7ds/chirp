...
#
# Bill Curlew (WCURLEW@JUNO>COM)
# FT450d.py base edited to work with FT-450 and FT-450AT older radios
#
...
@directory.register
class FT450Radio(yaesu_clone.YaesuCloneModeRadio):
    """Yaesu FT-450"""
    BAUD_RATE = 38400
    ...
    def get_features(self):
        ...
        #
        # need nostep_tuning to allow freqs loaded that had main tuning dial
        # adjustments. This is a quandry, as for SSB the main dial can be used
        # to set freq in 10hz steps, but the "step" with the multi function
        # knob is limited. Also in AM and FM mode, the main tuning knob is
        # disabled so only the multifunction step can be used.
        # Going to leave this off for now. Trying to specifiy a freq not
        # divisible by the steps will generate a gui error.
    ...
    def _get_special(self, number):
        ...
        if mem.number in range(self.FIRST_VFOA_INDEX,
            ...
            # ft450 only has 1 bit to expose PMS,
            # and does not set filled indicator
            bitindex = 0
    ...
    def _do_panel_buttons(self, pnlcfg):      # - - - Current Panel Config
        ...
        # Auto/Fast and Auto/Slow displayed on radio are governed by the mode.
        # both are encoded in memory as Auto (0)
