...
class BaofengDigital(chirp_common.CloneModeRadio,
    ...
    def _parse_tone(self, value):
        if value in (0x0000, 0xFFFF):
            return ('', None, 'N')

        # Parse lower 14 bit as BCD
        bcd_value = (
            1000 * (value >> 12 & 0x3) +
            100 * (value >> 8 & 0xF) +
            10 * (value >> 4 & 0xF) +
        ...
        if value & 0x8000:
            return ('DTCS', bcd_value, value & 0x4000 and 'R' or 'N')
        else:
            # Parse as CTCSS frequency * 10
            return ('Tone', bcd_value / 10.0, 'N')
    ...
    def get_memory(self, number):
        ...
        if not mem.empty:
            ...
            if raw.digital:
                mem.mode = raw.narrow and 'DN' or 'DIG'
                mem.tmode = ''
                ...
                tx_tmode, tx_tvalue, tx_tpol = self._parse_tone(raw.tx_tone)
                if tx_tmode == 'Tone':
                    mem.rtone = tx_tvalue
                elif tx_tmode == 'DTCS':
                    mem.dtcs = tx_tvalue

                rx_tmode, rx_tvalue, rx_tpol = self._parse_tone(raw.rx_tone)
                if rx_tmode == 'Tone':
                    mem.ctone = rx_tvalue
                elif rx_tmode == 'DTCS':
        ...
        rs = RadioSetting("encryption_key", "Encryption key",
                          RadioSettingValueInteger(0, 31, encryption_key))
        rs.set_doc('Encryption key. Only for digital modes.')
        mem.extra.append(rs)

        return mem

    def _pack_ctcss(self, freq):
        freq = int(freq * 10)
        # Pack in BCD
        return (
            0x1000 * (freq // 1000 % 10) |
    ...
    def _pack_dtcs(self, code, polarity):
        return (
            0x8000 |
            (polarity == 'R' and 0x4000) |
            0x1000 * (code // 1000 % 10) |
            0x100 * (code // 100 % 10) |
            0x10 * (code // 10 % 10) |
            code % 10
    ...
    def _pack_squelch(self, mode, ctcss_freq, dtcs_code, dtcs_polarity):
        if mode == 'Tone':
            return self._pack_ctcss(ctcss_freq)
        elif mode == 'DTCS':
            return self._pack_dtcs(dtcs_code, dtcs_polarity)
        else:
            return NO_TONE
    ...
    def set_memory(self, mem):
        ...
        raw.narrow = mem.mode in ('NFM', 'DN')
        raw.digital = mem.mode in ('DIG', 'DN')
        try:
        ...
        if raw.digital:
            ...
            raw.tx_tone = self._pack_ctcss(mem.rtone)
            raw.rx_tone = NO_TONE
        elif mem.tmode == 'TSQL':
            raw.tx_tone = raw.rx_tone = self._pack_ctcss(mem.ctone)
        elif mem.tmode == 'DTCS':
            ...
            raw.tx_tone = self._pack_dtcs(mem.dtcs, mem.dtcs_polarity[0])
            raw.rx_tone = self._pack_dtcs(mem.dtcs, mem.dtcs_polarity[1])
        elif mem.tmode == 'Cross':
            tx_tmode, rx_tmode = mem.cross_mode.split('->', 2)
            raw.tx_tone = self._pack_squelch(
                tx_tmode, mem.rtone, mem.dtcs, mem.dtcs_polarity[0])
            raw.rx_tone = self._pack_squelch(
                rx_tmode, mem.ctone, mem.rx_dtcs, mem.dtcs_polarity[1])
        ...
        for item in mem.extra:
            ...
            elif item.get_name() == 'encryption_key' and raw.digital:
                raw.encryption_key = item.value
...
# Disabled because UNTESTED, though it should work
# @directory.register
