...
class ICVT10Radio(icf.IcomCloneModeRadio):
    ...
    def _get_memory(self, number, extd_number):
        ...
        if extd_number is None:
            ...
            _usd = self._memobj.used[byte] if (number <= 205) else None
            _skp = self._memobj.skips[byte] if (number <= 205) else None
            _psk = self._memobj.pskips[byte] if (number <= 205) else None
    ...
    def _set_memory(self, mem):
        ...
        _usd = self._memobj.used[byte] if mem.number <= 205 else None
        _skp = self._memobj.skips[byte] if mem.number <= 205 else None
        _psk = self._memobj.pskips[byte] if mem.number <= 205 else None
