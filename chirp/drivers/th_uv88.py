...
#seekto 0x1160;
struct {
  char introScreen1[12];    // 0x1160 *Intro Screen Line 1(truncated to 12 alpha
                          //         text characters)
...
class THUV88Radio(chirp_common.CloneModeRadio):
    ...
    def get_settings(self):
        ...
        # software only
        rx = RadioSettingValueString(0, 16, _char_to_name(self._memobj.openradioname.name1))
        ...
        if self.MODEL != "RA89":
            rset.set_apply_callback(_name_apply,_settings,"introScreen1",self._memobj.openradioname,"name1")

        advanced.append(rset)

        # software only
        rx = RadioSettingValueString(0, 16, _char_to_name(self._memobj.openradioname.name2))
        rx.set_validate_callback(_name_validate)
